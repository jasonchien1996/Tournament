			...
			if (std::regex_match(command, match_move)) {
				...
				if (move == "?") {
	                ...
	                if (a.type() == action::place::type) {
		                int hint = host.at(id).evil->hint; // your hint tile here
		                if(hint > 3) hint = 4;
                	...
                	if (a.type() == action::place::type) {
                		host.at(id).play->hint = move[3] - '0';// move should be "PT+H" where H is '1', '2', '3', or '4'
                        host.at(id).play->total += 1;
                        int tile = move[1] - '0';
                        if(tile == 4) host.at(id).play->num_bonus += 1;
                        else host.at(id).play->bag[tile-1] -= 1;
                    }
					host.at(id).apply_action(a);
				}
